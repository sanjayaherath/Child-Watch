...
char mode[4] = {'M', 'C', 'P'}, buttonKeyArray [4] = {'8', '5', '2', '4'};
...
void checkButtonPress() {
  ...
  for (int pinVal = 6; pinVal < 10; pinVal++) {
    if (digitalRead(pinVal) == LOW && (millis() - pressedTime) > 150) {
      ...
      if (commandKey == '5' && disableMode == 0) modeIndex = (++modeIndex) % 3;
...
/*
  ...
  if (millis() - screenOffTimer > 60000 && screenOff != 1) {
...
//  if(screenOff == 1) display.clearDisplay();
}
...
void checkSms() {
  ...
  if (numberSms.equals(Number2) || numberSms.equals(Number2)) {
