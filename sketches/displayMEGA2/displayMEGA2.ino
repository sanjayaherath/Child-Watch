...
char contactInfo[13];
int screenOff = 0;
unsigned long screenOffTimer, checkMesTimer, sendMesTimer;
int sendMesCounter = 0, sendAcknow = 0;
...
void setup()   {
  ...
  //gsm  Sim800l.begin(); // initializate the library.
  //Sim800l.reset();
...
void loop() {
  ...
  if (optionKey == 'M' && sendAcknow == 0) {
    mainWindowTimeAndDate();
    mainWindowAnime();

    if (commandKey == '4') {
      switch (sendMesCounter) {
        case 0:
          if (millis() - sendMesTimer < 10000) break;
          sendMesTimer = millis();
          sendMesCounter++;
          break;
        case 1:
          if (millis() - sendMesTimer < 1000) {
            displayMesOLED("Message Sending");
            sendMessage(Number1, "Pick Me");
            sendMesCounter = 0;
            sendMesTimer = millis();
            optionKey = 'J';
          }
          else if (millis() - sendMesTimer > 500 && sendMesCounter != 0) {
            sendMesCounter = 0;
          }
          break;
      }
  ...
  if (millis() - checkMesTimer > 20000 && optionKey != 'S' && optionKey != 'C') {
    checkSms();
    checkMesTimer = millis();
  }
...
void checkButtonPress() {
  ...
  if (commandKey != '0' && screenOff == 0) {
    screenOffTimer = millis();
  }

  if (sendAcknow == 1 && commandKey != '0') {
    sendAcknow = 0;
    displayMesOLED("Message Sending");
    sendMessage(numberSms, "Acknowlege");
    modeIndex = 0;
    optionKey = 'C';
  }

  if (millis() - screenOffTimer > 30000 && screenOff != 1 && sendAcknow == 0) {
    display.clearDisplay();
    display.display();
    screenOff = 1;
  }
  else if (commandKey != '0' && screenOff == 1 ) {
    screenOff = 0;
    screenOffTimer = millis();
    modeIndex = 0;
    optionKey = 'C';
...
void checkSms() {
  ...
  if (numberSms.equals(Number1) || numberSms.equals(Number2)) {
    // Serial.println("match");
    displayMesOLED("Message Received");
    sendAcknow++;
    playSmsAlarm();
    // sendMessage();
  }
...
void displayMesOLED(char* textMessage) {
  display.clearDisplay();
...
void sendMessage(String sendNum, char* sendsmstext) {
  // send sms to contacts
  // char* sendsmstext = "match";
  error = Sim800l.sendSms(sendNum, sendsmstext);
  if (error) {
